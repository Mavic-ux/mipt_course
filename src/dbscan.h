...
auto dbscan(typename pcl::PointCloud<PointT>::Ptr input_cloud_ptr,  const float eps = 0.5f, const size_t minPts = 3){
...

    {
        ...
        for (size_t i = 0; i < input_cloud_ptr->size(); ++i){
            ...
            for (const int nbour_id : neighbours){
                ...
                bfs_queue.push(nbour_id);
            }


            while (bfs_queue.size() > 0){
                int index_q = bfs_queue.front();
                bfs_queue.pop();
